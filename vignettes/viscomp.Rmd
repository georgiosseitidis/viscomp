...
When multicomponent (complex) interventions are present in the network meta-analysis model we usually interested on: 

 * identifying the most efficacious components
 * identifying which component combination works better
 * understanding the behaviour of the components

The viscomp package provides several visualization tools to adress these issues. This document introduces you to viscomp’s set of tools, and shows you how to apply them on network meta-analysis model when multicomponent interventions (complex data) are present.
...
Network meta-analysis (NMA) model using the R-package `netmeta`. MACE data are dichotomous, and the NMA model is constructed using the odds ratios (OR) as effect size.
...
In NMA we usually have interventions that are used as a potentially inactive reference interventions (e.g. placebo, usual care), and are not combined with other interventions. We can exclude these interventions through the argument `excl`. For example, we can exclude the usual care (UC) from the component network plot with:
...
`heatcomp()` creates a heat plot comparing the two-by-two component combinations to the reference intervention. Diagonal elements refer to components, while off-diagonal to components combinations. Each element summarizes the efficacy of the interventions that includes the corresponding component combination. Combinations that were not observed in the NMA model, are denoted by the letter "X". The magnitude of the efficacy is reflected by the color's intensity. Estimates close to zero are denoted by white color, while above and below zero are denoted by warm colors (such as red) and cold colors (such as blue) respectively. The function by default uses the z-scores and the median as a summary measure. 
...
From the plot we see that the most intensive colors are observed for the component E, and the combination between components B and E, and A and E. Thus, these combination seems to be the most efficacious according to the `heatcomp()`.
...
Note that in the plot the median estimates of the components are equal with the diagonal elements of the `heatcomp()` function. Also note that we receive a warning message because of the component G, since it was included only in one intervention.
...
We can also group the violins based on the number of components. For example, we can create violins for interventions that includes 1 component, 2 components, 1-2 components and more than 2 components, with 
...
## Visualize the interventions' ranking for multiple outcomes with `hassePlot()`

`hassePlot()` is wrapper function of `netmeta::hasse`. More details about hasse plot can be found at <https://cran.r-project.org/web/packages/netmeta/>. 

\

We can create a hasse plot for the two outcomes (using the random-effects model for the first and the fixed-effect model for the second), with

```{r, fig.width = 7.2, fig.height = 6}
hassePlot(list(net1, net2), random = c(TRUE, FALSE))
```
